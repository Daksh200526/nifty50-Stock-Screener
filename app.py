...
# ── Build ticker maps ─────────────────────────────────────
all_ticker_map = dict(
    zip(df["Ticker"].astype(str), df["Company"].astype(str))
)

# ── Title ─────────────────────────────────────────────────
st.title("Nifty 50 Stock Screener")
st.caption(
    "Fundamental analysis tool — filter by P/E, market cap, sector & more"
)
...
filtered = filtered[
    filtered["Dividend Yield"] >= div_min
]
...
if week52 == "Near 52W High":
    filtered = filtered[
        filtered["Price"] >= filtered["52W High"] * 0.90
    ]

elif week52 == "Near 52W Low":
    filtered = filtered[
        filtered["Price"] <= filtered["52W Low"] * 1.10
    ]
...
col2.metric(
    "Green signals",
    len(filtered[filtered["Signal"] == "Green"])
)

col3.metric(
    "Yellow signals",
    len(filtered[filtered["Signal"] == "Yellow"])
)

col4.metric(
    "Red signals",
    len(filtered[filtered["Signal"] == "Red"])
)
...
# ── Download button ───────────────────────────────────────
csv = filtered[display_cols].to_csv(
    index=False
).encode("utf-8")
...
if selected_stock:
    ...
    stock_row = filtered[
        filtered["Ticker"] == selected_stock
    ].iloc[0]
    ...
    m2.metric(
        "P/E Ratio",
        f"{stock_row['P/E Ratio']:.1f}"
        if pd.notna(stock_row["P/E Ratio"])
        else "N/A"
    )

    m3.metric(
        "P/B Ratio",
        f"{stock_row['P/B Ratio']:.2f}"
        if pd.notna(stock_row["P/B Ratio"])
        else "N/A"
    )

    m4.metric(
        "Dividend Yield",
        f"{stock_row['Dividend Yield']:.2f}%"
        if pd.notna(stock_row["Dividend Yield"])
        else "N/A"
    )
    ...
    st.markdown(
        f"#### {stock_row['Company']} — 6 month price chart"
    )
    ...
    if hist is not None and not hist.empty:
        ...
        hist["RSI"] = ta_lib.momentum.RSIIndicator(
            hist["Close"],
            window=14
        ).rsi()
        ...
        if chart_type == "Price + Moving Averages":
            ...
            with st.expander(
                ...
                st.markdown("""
                - **20-day MA** — average closing price over last 20 days.
                - **50-day MA** — average closing price over last 50 days.
                - **Golden Cross** — MA20 crossing above MA50 is bullish.
                - **RSI** — see RSI section below.
                """)
        ...
        st.warning(
            "Chart data not available for this stock."
        )
...
if len(selected_companies) >= 2:
    ...
    for company in selected_companies:
        ...
        pe_score = (
            max(0, 10 - (row["P/E Ratio"] / 5))
            if pd.notna(row["P/E Ratio"])
            else 0
        )

        pb_score = (
            max(0, 10 - (row["P/B Ratio"] * 1.5))
        ...
        div_score = (
            min(10, row["Dividend Yield"] * 3)
            if pd.notna(row["Dividend Yield"])
            else 0
        )

        comp = (
            row["Composite Score"] / 10
            if pd.notna(row["Composite Score"])
            else 0
        )
